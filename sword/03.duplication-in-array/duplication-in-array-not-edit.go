...
方法一：创建一个长度为n+1辅助数组，如果原数组中被复制的是m，则把它复制到辅助数组中下表为m的位置。这样很容易发现哪个数字是重复的，空间复杂度O(n)。
方法二：假设没有重复数字，那么在1n的范围内只有n个数字，超过则有重复。我们把1n的数字从中间数字m分为两部分，前面一半为1m后一半为m+1n。
...
func findDuplicationNumberInArrayNotEdit(numbers []int, length int) int {
	if length <= 0 {
		return -1
	}
